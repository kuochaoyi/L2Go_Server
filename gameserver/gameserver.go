...
func Create(cfg config.GameServerConfigObject) *GameServer {
...
func (g *GameServer) Init() {
	...
	if err != nil {
		log.Print("Couldn't connect to the Login Server")
	} else {
		log.Printf("Successfully connected to the Login Server at %s:9413\n", g.config.LoginServer.Host)
	}

	// Listen for client connections
	g.clientListener, err = net.Listen("tcp", ":"+strconv.Itoa(g.config.GameServer.Port))
	if err != nil {
		log.Print("Couldn't initialize the Game Server")
	} else {
		log.Printf("Game Server listening on port %s\n", strconv.Itoa(g.config.GameServer.Port))
	}
}
...
func (g *GameServer) handleClientPackets(client *models.Client) {
	...
	if protocolVersion.Version < 419 {
		log.Println("Wrong protocol version ! <Expected 419> <Got: %d>", protocolVersion.Version)
		return
	}

	log.Println("Sending the Xor Key to the client...")
	...
	if err != nil {
		...
		log.Println("CryptInit packet sent.")
	}
	...
	for {
		...
		switch opcode {
		case 0x08:
			log.Println("Client is requesting login to the Game Server")
		...
		case 0x0e:
			log.Println("Client is requesting character creation template")
		...
		case 0x0b:
			...
			log.Printf("Created a new character : %s\n", character.Name)
